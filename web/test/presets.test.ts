...
test('练习「牵引机控制回路」：延时→启动自锁+风机→选挡互锁→电机转→停机', () => {
  ...
  // 全断待机：QF1 未合，指示灯不亮，也无误报（分闸是正常操作态）
  const states: StateById = {};
  let r = simulate(buildCircuit(p, states));
  assert.equal(on(r, 'HL1'), false, 'QF1 未合指示灯不应亮');
  assert.equal(on(r, 'M'), false);
  noErrors(r, '全断待机');

  // 合 QF1：指示灯亮；QF2 未合，按 SB1 无效
  states.QF1 = { closed: true };
  r = simulate(buildCircuit(p, states));
  assert.equal(on(r, 'HL1'), true, '合 QF1 后指示灯应亮');
  states.SB1 = { pressed: true };
  states.KT1 = { closed: true };
  r = simulate(buildCircuit(p, states));
  assert.equal(energized(r, 'KM1_C'), false, 'QF2 未合控制回路无电，不应能启动');
  states.SB1 = { pressed: false };
  states.KT1 = { closed: false };

  // 合 QF2 后：ZN96 本体得电（UI 层据此开始计时）；延时没到时按 SB1 无效
  states.QF2 = { closed: true };
  states.SB1 = { pressed: true };
  r = simulate(buildCircuit(p, states));
  assert.equal(energized(r, 'KT1_B'), true, '合 QF2 后 ZN96 本体应得电开始计时');
  assert.equal(energized(r, 'KM1_C'), false, '延时未到不应能启动');
  // 延时到（UI 把同组延时触点闭合）
  states.KT1 = { closed: true };
  r = simulate(buildCircuit(p, states));
  assert.equal(energized(r, 'KM1_C'), true, '延时到后按住 SB1 应能启动');

  // 延时到 + 按住 SB1：KM1 吸合、风机转；松开自锁
  r = simulate(buildCircuit(p, states));
  ...
  // 选 KM2 挡 → 电机转（60V 抽头 → 直流 ≈81V，全速；表计有读数）
  persist(states, r);
  states.SA2_1 = { closed: true };
  r = simulate(buildCircuit(p, states));
  assert.equal(energized(r, 'KM2_C'), true);
  assert.equal(on(r, 'M'), true, 'KM1+KM2 都吸合，电机应转');
  const m60 = r.components.find((c) => c.id === 'M')!;
  assert.ok(Math.abs((m60.volts ?? 0) - 81) < 3, `60V 挡直流电压应≈81V，实际 ${m60.volts}`);
  assert.equal(m60.speedPct, 100, '60V 挡应全速');
  const pv = r.components.find((c) => c.id === 'PV1')!;
  assert.ok((pv.volts ?? 0) > 75, `电压表应有读数，实际 ${pv.volts}`);
  const pa = r.components.find((c) => c.id === 'PA1')!;
  assert.ok((pa.amps ?? 0) > 3, `电流表应有读数，实际 ${pa.amps}`);
  noErrors(r, '运行中');
  persist(states, r);
  states.SA2_2 = { closed: true };
  r = simulate(buildCircuit(p, states));
  ...
  // 换到 KM3 挡（30V 抽头）：直流 ≈40.5V，转速降半
  persist(states, r);
  states.SA2_1 = { closed: false };
  r = simulate(buildCircuit(p, states));
  persist(states, r);
  r = simulate(buildCircuit(p, states));
  assert.equal(energized(r, 'KM3_C'), true, '退出 KM2 挡后 KM3 应吸合');
  const m30 = r.components.find((c) => c.id === 'M')!;
  assert.equal(on(r, 'M'), true, '30V 挡电机应仍在转');
  assert.ok(Math.abs((m30.volts ?? 0) - 40.5) < 3, `30V 挡直流电压应≈40.5V，实际 ${m30.volts}`);
  assert.ok(Math.abs((m30.speedPct ?? 0) - 50) <= 3, `30V 挡应约半速，实际 ${m30.speedPct}%`);

  // 按 SB2 停机：KM1 断 → 电机停、风机停（选挡支路独立，KM3 仍吸合）
  persist(states, r);
  ...
  assert.equal(energized(r, 'KM3_C'), true, '选挡支路独立于 KM1（与原图一致）');
});
